...
function getBit(num: number, postion: number) : string{
    const bitmaskRedult = 1 << postion;
    const result =  bitmaskRedult & num;
    if(result == 0){
        return "Bit was 0";
    }else{
        return "Bit was 1";
...
function setBit(num: number, postion: number) : number{
    const bitmaskRedult = 1 << postion;
